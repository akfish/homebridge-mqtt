...
const HISTORY_SERVICE_UUID = 'E863F007-079E-48FF-8F27-9C2605A29F52';
...
function Accessory(params) {
  ...
  HistoryService = params.HistoryService;
...
Accessory.prototype.configureAccessory = function(accessory, api_accessory, service_name, service_type) {
  ...
  // TODO: deferred creation when characteristics are availiable.
  this.log.info(`Initializing HistoryService for '${this.name}'`);
  this.historyService = new HistoryService("weather", accessory, { storage: "fs" });
  this.services[this.historyService.displayName] = this.historyService;
}
...
Accessory.prototype.configureCharacteristics = function(accessory, api_accessory, service_name) {
  ...
  // Skip HistoryService
  if (service.UUID === HISTORY_SERVICE_UUID) return;
  ...
  if (result.isValid) {
    ...
    switch (c) {
      case "CurrentTemperature":
        ...
        this.log.info("History.addEntry({ time: %s, temp: %s })", t, value);
        break;
      case "CurrentRelativeHumidity":
        ...
        this.log.info("History.addEntry({ time: %s, humidity: %s })", t, value);
        break;
    }
